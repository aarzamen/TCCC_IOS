...
/// generators (radio, narrative, ZMIST) can swap to a different
/// backend at runtime per night-pass Track C.
///
...
/// `ZMISTNarrativeGenerator`) all consume `any TCCCLLMBackend` per
/// call (vended by `AppState.currentBackend`), so flipping the
/// Settings backend toggle swaps engines at runtime.
