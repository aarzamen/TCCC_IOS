...
- Kokoro synthesis intentionally fails closed because `/Users/ama/Kokoro-82M` has PyTorch `.pth` and `.pt` tensors only. No Core ML, MLX Swift, Swift/iOS G2P, or license bundle is available yet.
- Readout uses preserved script text, sentence-level timing metadata when real Kokoro output exists, and `AVAudioPlayer` metering only when a real audio URL exists. It does not fake playback levels.
...
- Issue: acceptance criteria include native Kokoro playback, actual highlighting, waveform, speed/pitch effects, and memory profiling.
  Fix: documented these as blocked by the missing native Kokoro runtime rather than shipping fake audio, platform TTS, or silent success.
...
- No Kokoro audio playback occurs until the PyTorch-only clone is converted or replaced with a confirmed native Core ML / MLX Swift path.
